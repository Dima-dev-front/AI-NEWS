...
class Summarizer:
    ...
    def select_best(self, items: list[dict]) -> list[int]:
        ...
        list_text = "\n".join(lines)
        prompt = (
            "Ти — головний редактор науково-технологічного видання. З наведеного списку обери 1 НАЙКРАЩУ новину, \n"
            "враховуючи глобальну важливість, актуальність, потенційний вплив на суспільство та прогрес. \n"
            "ПРІОРИТЕТНІ ТЕМИ: ІТ/ШІ, наукові відкриття, космос, технології, медицина, екологія. \n"
            "Обирай різноманітні теми - не лише ШІ, але й науку, космос, нові технології. \n"
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                model=self.model_name,
                messages=[
                    {"role": "system", "content": "Ти головний редактор науково-технологічного видання. Обираєш 1 найкращу новину з різних тем: ІТ/ШІ, наука, космос, технології. Відповідаєш ТІЛЬКИ валідним JSON."},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.2,
                max_tokens=200,
